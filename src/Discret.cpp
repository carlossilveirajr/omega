...
void quicksort(Instance * vet, int low, int high){
  ...
  while(low<=high) {
      while (vet[low].val<pivo) low++;
      while (vet[high].val>pivo) high--;
      if(low<=high){
        temp = vet[low].val;
        cattemp = vet[low].cat;
  ...
  if(lowold<high) quicksort(vet,lowold,high);
  if(highold>low) quicksort(vet,low,highold);
}
...
void rank(Attribute * vet, int low, int high){
  if(low>=high) return;
  int cattemp;
  float pivo = vet[(low+high)/2].incrate, lowold=low, highold= high, temp;
  while(low<=high) {
      while (vet[low].incrate<pivo) low++;
      while (vet[high].incrate>pivo) high--;
      if(low<=high){
  ...
  if(lowold<high) rank(vet,lowold,high);
  if(highold>low) rank(vet,low,highold);
}
...
int getMajority(int nbrlines, Attribute &attrib,float left_close, float right_open, int nbrcats,  int &nbrinst, int &nbrmaj){
     int inst=0;
     ...
     nbrinst=0;
     while( (inst < nbrlines) && (attrib.instances[inst].val >= left_close)  && (attrib.instances[inst].val < right_open))
...
void getEdgesCondition1( Attribute &attrib, int nbrlines){
         ...
         for(int i=1;i<nbrlines-1;i++)
               if(attrib.instances[i-1].cat!=attrib.instances[i].cat)
                    if(attrib.instances[i-1].val + 0.000000009 < attrib.instances[i].val)
...
void getEdgesCondition2( Attribute &attrib, int nbrlines, int binsize){
     ...
     for(iedg=1;iedg<attrib.nbredges-1; iedg++){
         while(( attrib.instances[inst].val < attrib.edges[iedg])  && (inst< nbrlines-1)){ //last edge mantained
             bincount++;
             inst++;
         }
         if(bincount>= binsize){
...
**/
void getEdgesCondition3( Attribute &attrib, int nbrlines, int nbrcats,  float incrate){
     ...
     for(iedg=1;iedg<attrib.nbredges; iedg++){
          ...
          maj = getMajority(nbrlines,attrib,left_close,right_open,nbrcats, nbrinst, nbrmaj);
          fused = false;
...
void calcInconsistence( Attribute &attrib, int nbrlines, int nbrcats){
     ...
     for(iedg=1;iedg<attrib.nbredges; iedg++){
          ...
          if(iedg+1 == attrib.nbredges)
               right_open+=0.01; //closed interval, last cut point
          getMajority(nbrlines,attrib,left_close,right_open,nbrcats, nbrinst, nbrmaj);
...
*
*/
void discretize(Reader * reader,int binsize, float incratetofuse, int nbrcats, float incratetoselect){

   FILE * f = fopen("presage_log.txt","w");
   fprintf(f,"binsize=%d; max_inc_to_fuse=%.2f; nbrcats=%d; max_globalinc_to_select=%.3f",binsize,incratetofuse,nbrcats, incratetoselect);
   ...
   //it stores instances value and class info for each attribute.
    ...
    for(int iat=0;iat<nbrattribs;iat++){ //begin
         ...
         do{
           ...
           attributes[iat].instances[i].val  = reader->getValueOfIndex(reader->getQuanValue(), iat);
           i++;
         ...
         for(int i=0;i<nbrlines;i++)
            printf("\ncat=%d val=%.6f", attributes->instances[i].cat, attributes->instances[i].val);
...
//   printf("\nProgram ended. time spent=%.5f seconds.",(end - start) / CLK_TCK);
//   fprintf(f,"\nTime spent=%.5f seconds",(end - start) / CLK_TCK);
   fclose(f);
}//end function
...
void transformNOMINAL(Reader * reader, Attribute * attributes,  int nbrcats){
    ...
    //cabecalho do arquivo do weka ==> COMENTEI AQUI PARA TIRAR O CABECALHO DO WEKA
  /*  fprintf(fd,"@relation discweka\n");
    fprintf(fd,"@attribute id NUMERIC \n");
    //atributos
    for(iat=0;iat<nbrattribs;iat++){
       fprintf(fd,"@attribute a%i {",iat+1);
            for(bin=1; bin< attributes[iat].nbredges-1;bin++)
            fprintf(fd,"%i[%.8f-%.8f[,",iat+1,attributes[iat].edges[bin-1],attributes[iat].edges[bin]);
         fprintf(fd,"%i[%.8f-%.8f[}\n",iat+1,attributes[iat].edges[bin-1],attributes[iat].edges[bin]);
     }
     ...
     */
     ...
     do{
             ...
             for(iat=0;iat<nbrattribs;iat++){
                 ...
                 upper=1;
                 while( (upper < attributes[iat].nbredges) && ( attributes[iat].edges[upper] < val +TOLERATE))
                     upper++;
                 fprintf(fd,"%i[%.8f-%.8f[ ",iat+1,attributes[iat].edges[upper-1],attributes[iat].edges[upper]);
              }
